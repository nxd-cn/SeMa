...
- **前端缓存 / 自动更新**：打包后禁用了页面右键，无法手动刷新。Win WebView2 / Mac WKWebView 可能缓存旧 `index.html` 与 hashed 资源。正式包在 `package` 版本相对 `webview-asset-version.txt`（app data）变化时 `clear_all_browsing_data` + `reload`（`webview_cache.rs`；`debug_assertions` 跳过）。发版须抬版本号（CI 已从 tag 写回），同版本重装不会再清一次
