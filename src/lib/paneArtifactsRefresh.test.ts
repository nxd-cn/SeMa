...
describe("runArtifactsRefresh", () => {
  ...
  it("fetches when forced even if under the throttle window", async () => {
    ...
    const r = await runArtifactsRefresh({
      query: { cliId: "claude", cwd: "/proj", cliSessionId: "s1" },
      force: true,
      lastFetchMs: 1000,
      nowMs: 1500,
      sessionArtifacts,
  ...
  it("fetches a throttled idle refresh when elapsed is at least 2000ms", async () => {
    const sessionArtifacts = vi.fn(async () => sample);
    const r = await runArtifactsRefresh({
      query: { cliId: "cursor", cwd: "/p", cliSessionId: "s2" },
      force: false,
      lastFetchMs: 1000,
      nowMs: 3000,
      sessionArtifacts,
...
describe("shouldShowLoading", () => {
  ...
  it("shows loading only for force-refresh when the panel is already expanded", () => {
    expect(shouldShowLoading(true, true)).toBe(true);
  });

  it("does not show loading for idle refresh even when expanded", () => {
